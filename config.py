...
PUBLIC_BASE_URL = env("PUBLIC_BASE_URL").strip().rstrip("/")
...
CRON_SECRET = env("CRON_SECRET").strip()
BLOB_READ_WRITE_TOKEN = env("BLOB_READ_WRITE_TOKEN").strip()
...
TOKEN_ENCRYPTION_KEY = (env("TOKEN_ENCRYPTION_KEY") or env("SESSION_SECRET")).strip()
...
def validate_runtime_config() -> None:
    ...
    if GOOGLE_CALENDAR_ENABLED:
        ...
        if IS_PRODUCTION and not env("TOKEN_ENCRYPTION_KEY"):
            raise RuntimeError(
                "TOKEN_ENCRYPTION_KEY must be set explicitly when Google Calendar is enabled"
            )
